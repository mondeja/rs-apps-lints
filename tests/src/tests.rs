...
impl LintDirectories {
    fn new() -> Self {
        let lints_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("..")
            .join("lints");
        let lints_libraries_dirs = std::fs::read_dir(lints_dir).expect("Failed to read lints directory");
        ...
        for entry in lints_libraries_dirs {
            if let Ok(entry) = entry {
                ...
                let lint_dirs = std::fs::read_dir(&path)
                    .expect("Failed to read lint directory");
...
fn lints_readmes_are_updated() {
    ...
    let mut updated = vec![];
    for LintDirectory{ name, path, library_name } in LintDirectories::new() {
        ...
        let doc_comment = extract_lint_lib_doc_comment(&lint_lib_rs_content).unwrap_or_else(|| {
            panic!("Failed to extract documentation comment from lib/{library_name}/{name}/src/lib.rs",)
        });
...
fn lints_have_help_link() {
    for LintDirectory{ name, path, library_name } in LintDirectories::new() {
        ...
        let expected_link =
            format!("https://github.com/mondeja/rs-apps-lints/tree/main/lints/{library_name}/{name}#readme");
        assert!(
